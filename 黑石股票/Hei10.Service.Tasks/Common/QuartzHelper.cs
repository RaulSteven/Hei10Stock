...
namespace Hei10.Service.Tasks.Common
{
    /// <summary>
    /// 任务处理帮助类
    /// </summary>
    public class QuartzHelper
    {
        ...
        public static void ScheduleJob(JobTask taskUtil)
        {
            ...
            if (taskUtil.CommonStatus != CommonStatus.Deleted)
            {
                //验证是否正确的Cron表达式
                if (ValidExpression(taskUtil.CronExpressionString))
                {
                    ...
                    if (taskUtil.IsDeleteOldTask)
                    {
                        ...
                        _jobTaskRepository.SaveAsync(taskUtil);
                    }
                }
                else
                {
        ...
        {
            if (taskUtil.CommonStatus == CommonStatus.Disabled)
            {
                TaskManager.scheduler.PauseJob(jk);
            }
            else if (taskUtil.CommonStatus == CommonStatus.Deleted)
            {
                //删除已存在任务
                DeleteJob(taskUtil.TaskId.ToString());
            }
